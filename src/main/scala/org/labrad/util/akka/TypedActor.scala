package org.labrad.util.akka

import language.existentials

import akka.actor.{Actor, ActorContext, ActorRef, ActorRefFactory, ActorSystem, Deploy, ExtendedActorSystem, Extension, ExtensionId, ExtensionIdProvider, PoisonPill, Props, Status, SupervisorStrategy}
...
trait TypedActorFactory {
  ...
  //  case null => false
  //  case ref  => ref.asInstanceOf[akka.actor.InternalActorRef].stop; true
  //}
  ...
  def typedActorOf[R <: AnyRef, T <: R](props: TypedProps[T]): R = {
    ...
    val ap = Props(new TypedActor.TypedActor[R, T](proxyVar, c())).withDeploy(props.actorProps.deploy)
...
object TypedActor extends ExtensionId[TypedActorExtension] with ExtensionIdProvider {
  ...
  case class MethodCall(method: Method, parameters: Array[AnyRef]) {
    ...
    def timeoutOpt: Option[Timeout] = Option(parameters).toSeq.flatten.lastOption flatMap {
      case timeout: Timeout => Some(timeout)
      case _ => None
    }
   ...
   */
  private case class SerializedMethodCall(ownerType: Class[_], methodName: String, parameterTypes: Array[Class[_]], serializedParameters: Array[(Int, Class[_], Array[Byte])]) {
  ...
  private[akka] class TypedActorInvocationHandler(@transient val extension: TypedActorExtension, @transient val actorVar: AtomVar[ActorRef], @transient val timeout: Timeout) extends InvocationHandler with Serializable {
      ...
      case _ =>
        ...
        MethodCall(method, args) match {
          ...
          case m if m.returnsFuture =>
            val t = m.timeoutOpt.getOrElse(timeout)
            ask(actor, m)(t) map {
          ...
          case m =>
            val t = m.timeoutOpt.getOrElse(timeout)
            Await.result(ask(actor, m)(t), t.duration) match {
              case NullResponse => null
    ...
    @throws(classOf[ObjectStreamException])
    private def writeReplace(): AnyRef = SerializedTypedActorInvocationHandler(actor, timeout.duration)
  }

  /**
   * INTERNAL API
   */
  private case class SerializedTypedActorInvocationHandler(val actor: ActorRef, val timeout: FiniteDuration) {
    @throws(classOf[ObjectStreamException])
...
object TypedProps {
  ...
  def extractInterfaces(clazz: Class[_]): immutable.Seq[Class[_]] =
    if (clazz.isInterface) immutable.Seq(clazz) else immutable.Seq(clazz.getInterfaces.toSeq: _*)
  ...
  def apply[T <: AnyRef : ClassTag](creator: => T): TypedProps[T] = {
    val clazz = implicitly[ClassTag[T]].runtimeClass.asInstanceOf[Class[T]]
    new TypedProps[T](immutable.Seq(clazz), () => creator)
  }
  ...
  def apply[T <: AnyRef: ClassTag](): TypedProps[T] = {
    val clazz = implicitly[ClassTag[T]].runtimeClass.asInstanceOf[Class[T]]
    new TypedProps[T](immutable.Seq(clazz), () => instantiate(clazz))
  }
   ...
   */
  private def instantiate[T](clazz: Class[T]): T = try clazz.newInstance catch {
...
class TypedActorExtension(val system: ExtendedActorSystem) extends TypedActorFactory with Extension {
   ...
   */
  final val DefaultReturnTimeout = Timeout(Duration(settings.config.getDuration("akka.actor.typed.timeout", MILLISECONDS), MILLISECONDS))
