...
object Tx {
  ...
  val aborted: Tx[Nothing] = new Tx[Nothing] {
    def ack() = Future.successful(Abort)
    def nack() {}
  }
  ...
  def const[T](msg: T): Tx[T] = new Tx[T] {
    def ack() = Future.successful(Commit(msg))
    def nack() {}
  }
      ...
      def ack(): Future[Result[T]] = lock.synchronized {
        state match {
          case Idle =>
            ...
            state = Ackd(this, {
              case true => p.trySuccess(Commit(msg))
              case false => p.trySuccess(Abort)
            })
            p.future
          ...
          case Ackd(who, confirm) if who ne this =>
            confirm(true)
            state = Done
            Future.successful(Commit(msg))

          case Nackd(who) if who ne this =>
            state = Done
            Future.successful(Abort)
