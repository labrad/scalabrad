...
object Setter {
  ...
  implicit def tuple2Setter[T1, T2](implicit s1: Setter[T1], s2: Setter[T2]) = new Setter[(T1, T2)] {
    def set(data: Data, value: (T1, T2)) {
      ...
      s1.set(data(0), value._1)
      s2.set(data(1), value._2)
    }
  }
