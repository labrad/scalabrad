package org.labradpackage
package types

import scala.math

trait Dimensional {
  def toSI: Double => Double
  def fromSI: Double => Double
}

object dBm extends Dimensional {
  def toSI = (v: Double) => math.pow(10, v / 10)
  def fromSI = (v: Double) => 10 * math.log(v) / math.log(10)
}

object degC extends Dimensional {
  def toSI = (v: Double) => v + 273.15
  def fromSI = (v: Double) => v - 273.15
}

object degF extends Dimensional {
  def toSI = (v: Double) => degC.toSI((v - 32) * 4/9)
  def fromSI = (v: Double) => degC.fromSI(v) * 9/4 + 32
}
